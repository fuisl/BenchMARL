...
**Done when:** every row above has a question, intervention, control, metric, and task rationale — met. **Next action:** finish the Give Way pipeline-calibration check, then the Buzz-Wire behaviour-policy check, then M2 (simulator snapshot/restore + oracle CEM-MPC) on whichever task passes.
