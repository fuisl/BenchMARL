...
| 1525 | `stage0_admission_1525/` | **Horizon sweep**, true simulator only, 5 scenarios x 3 seeds at h=1,2,3,5. **Balance's interaction is temporally mediated**: `J_cross` 0.049 -> 0.439 -> 1.065 -> 1.110, activity 17% -> 72%. Transport stays ~8%, Wheel ~5%, Dropout exactly 0.000 at every horizon. COMPLETED 11:21 | [`33`](33_admission_benchmark.md) |
| 1527 | `balance_representation_1527/` | **Balance representation audit at h=3. NO MEASUREMENT.** Both cells returned `E ~ 1e10` with train matching test, so not overfitting: `E_CF` averages per-anchor ratios and 44% of Balance anchors carry ~no effect. Buzz Wire's 100%-active, median~mean distribution had masked the fragility. Fix registered (ratio of sums); `O >> Z` reproduction remains open. COMPLETED 2:54:05 | [`33`](33_admission_benchmark.md) |
...
| 1538 | `ecf_agreement_1538/` | **Aggregation gate PASSED**: pooled vs mean-of-ratios on the T-A2b configuration. `O/Z` stable to 0.6%; `+agent-physical` 0.542 → 0.402. Pooled becomes the frozen convention | [`33`](33_admission_benchmark.md) |
| 1540 | `balance_representation_1540/` | Balance audit re-run on cells `1:1`, `0:1`: bounded under pooling but **uninformative** (reference no better than blind). The wrong cells were inherited | [`33`](33_admission_benchmark.md) |
| 1541 | `admission_gate_1541/` | **A-vs-S admission gate**, 5 scenarios × h ∈ {1,2,3,5}. Admits Buzz Wire h = 1 and Balance h = 2/3/5 cell `1:0`; Transport and Wheel rejected; one false admission (Buzz Wire h = 3, one anchor) | [`33`](33_admission_benchmark.md) |
| 1543, 1544 | `ta2_reference_baselines_sigreg0009_1543/`, `…sigreg09_1544/` | T-A2 bank retrained at SIGReg λ = 0.009 / 0.9, 48 checkpoints each. COMPLETED | [`34`](34_convention_audit_scaling_balance.md) |
| 1545 | `ta2_f13_corrected_1545/` | **Canonical 1498 bank re-scored under pooled `E_CF`**, h = 1–3. H1 beats H0 **8/8** at h = 1, 2 (MoR on the same fits: 0/8). Axis 0 is resolved (E 0.66, cos +0.80); axis 1 is not. Relational vs joint is null. **Re-opens K21/K22/K24.** COMPLETED | [`34`](34_convention_audit_scaling_balance.md) |
| 1546, 1547, 1550, 1553 | `encoder_confound_*` | Encoder audits at λ = 0.09 / 0.009 / 0.9 with untrained-encoder and latent ⊕ observation controls. Retention is monotone in λ (0.07–0.44), and about half the observation-to-latent gap is the diagnostic head. 1551/1552 were cancelled and folded into 1553 | [`34`](34_convention_audit_scaling_balance.md) |
| 1548, 1549 | `ta2_f13_corrected_1548/`, `…1549/` | T-A2 h = 1 on the λ banks. λ = 0.009 has the highest retention and the worst fidelity (0.92–0.98); λ = 0.9 ≈ reference. COMPLETED | [`34`](34_convention_audit_scaling_balance.md) |
| 1554, 1555 | — | Failed in 23 s: `/home` full. `outputs/` moved to `/data/fuisloy/benchmarl_outputs` and symlinked | [`34`](34_convention_audit_scaling_balance.md) |
| 1560, 1562, 1563 | `component_scaling_1560/`, `ta2_f13_corrected_1562/`, `encoder_confound_scaling_1563/` | **Component scaling**, 15 cells × 4 seeds. No capacity knob improves `E_CF` by > 0.033; removing data costs +0.03 to +0.07 on 16/16 seed-analyses. S2 supported, S1 fails for the encoder. COMPLETED | [`34`](34_convention_audit_scaling_balance.md) |
| 1561, 1564, 1565 | `ta2_reference_baselines_balance_1561/`, `ta2_f13_corrected_1564/`, `encoder_confound_balance_h3_1565/` | **Balance replication** on admitted cell `1:0`. H1 vs H0 0/8; relational beats joint 8/8 by shrinkage (gain 0.14 vs 0.30). **No cell passes the probe-floor rule.** Encoder audit unresolvable (gap 0.12 vs ±0.03). COMPLETED | [`34`](34_convention_audit_scaling_balance.md) |
| 1566 | `single_agent_gate_1566/` | **Single-agent gate** on VMAS navigation (1 agent, linear dynamics): 6 cells × 3 seeds, independent `lewm_reference`. Rollout beats persistence at all 8 horizons in 5/6 cells (MLP probe); self-response has the right direction and size from the 2nd changed block on; the first block is imprecise (G2 missed). 300 epochs halves every error, so the T-A2 recipe (600 updates on Buzz Wire) is under-optimized. COMPLETED 32:14 | [`35`](35_single_agent_gate.md) |
| 1569 | — | First single-agent control submission. Cancelled: six processes × 3–6 models × 64 episodes were too slow. Produced nothing | [`36`](36_single_agent_control.md) |
| 1570, 1571 | `single_agent_control_1570/`, `…_1571/` | **Single-agent control** on VMAS navigation with LeWM's CEM protocol and latent goal distance, on the 1566 checkpoints. Learned planner 92–100% in every recipe (base 190/192, 300 epochs 176/192) against oracle and PD 100%, random ≤ 1/64; the reward-head cost scores 2/16. Failures are far-goal stalls (`d0` ≥ 1.06). Better prediction did not give better control. COMPLETED | [`36`](36_single_agent_control.md) |
...
| `33_admission_benchmark.md` | Cross-scenario admission: frozen pooled convention, A-vs-S admission gate, Balance admitted on axis 0 (graded and found below probe resolution in 34) |
| `34_convention_audit_scaling_balance.md` | **Closes the VMAS phase**: pooled re-scoring re-opens K21/K22/K24, SIGReg sweep, component scaling, Balance replication, head-capacity control, the consolidated mistake list |
| `35_single_agent_gate.md` | Single-agent gate: the independent latent model rolls out and responds to one agent's own action; the T-A2 recipe is under-optimized by 8-24x in updates |
| `36_single_agent_control.md` | Single-agent control: LeWM-protocol latent-goal CEM completes navigation at 92–100%; the reward head does not; prediction accuracy does not predict control |
