...
| C7 | Models are comparable in-distribution but differ counterfactually | **7.1–7.2** | $E_{\mathrm{ID}}$ parity + $G_{\mathrm{CF}}$ separation | **HAVE (Buzz Wire) · NEGATIVE (Transport)** | 05_counterfactual_prediction.md. Buzz Wire (job 1196): relational captures 26% of the cross-agent effect vs the no-response floor, 8/8 seeds both regimes, and beats joint (identical information, different inductive bias) on 7/8 with CIs excluding zero. Transport: 1.002x the floor, i.e. nothing. In-distribution parity holds on both |
...
| C10 | Benefit tracks measured cross-agent effect, not task identity | 7.5, 8 | Same comparison across ≥2 interaction mechanisms + weak-coupling control | **HAVE** | Three tasks spanning the coupling range with identical code, budgets, capacity and seeds: Dropout (no dynamics coupling) no effect; Transport (14% of anchors, none at the first step) relational captures 0%; Buzz Wire (100%, all at the first step) relational captures 26%. The benefit appears exactly where the measured coupling does |
