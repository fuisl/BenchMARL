...
| K28 | The counterfactual deficit is the **omitted mediating state**, not the encoder or the JEPA objective. | **Established (jobs 1506, 1508)** — now with a passed heterogeneity check across all 48 checkpoints, three architectures and two regimes: every latent arm within 0.015 of the blind floor, every `+agent-physical` arm within 0.02, every `+ball` arm within 0.004 at `R ≈ 0.86`. No architecture, seed or regime deviates, so the deficit is a property of the observation, invariant to everything varied inside the model. | Adding the ball/linkage state to the same latent, through the same head, recovers **86-88%** of the blind-to-ceiling gap on cross `E_CF` (0.881-0.895 -> 0.517-0.523 against a 0.4645 ceiling) and **96%** on cosine. Buzz Wire's observation is `[pos, vel, pos - goal]`, verified against the installed vmas source: no ball, no linkage, no partner, while the ball is rigidly jointed to both agents and mediates the entire cross-agent effect. This makes an agent-neutral world token the indicated repair rather than a design preference. Bounded to one task and `h=1`. [32](paper/experiments/32_joint_representation.md) |
| K36 | The encoder is exonerated, because the raw observation scores no better than the latent. | **Withdrawn (job 1516)** | Under a per-cell cross-fitted head the raw observation reaches **0.605** against the latent's **0.214**: the observation carries a clear majority of the recoverable effect and the encoder loses about two thirds of it. The exoneration rested entirely on the two being equal under the pooled full-`ΔY` convention. This is a JEPA-abstraction result and reverses what note 30 recorded. [30](paper/experiments/30_task_a_counterfactual_fidelity.md) |
...
| K25 | The T-A2 counterfactual failure is attributable to the predictor `P`. | **Rejected (job 1503)** — but see K36: its companion claim that the *encoder* is also exonerated is withdrawn. Job 1503 localized the loss upstream of `P`; job 1516 shows a large part of it is the encoder rather than the observation. | The effect is already lost before `P` runs. A head fitted on the current latent recovers +0.00 to +0.03 of the blind-to-ceiling gap on cross `E_CF`, while the true physical state through the identical head reaches 0.4645. Test B (`probe_floor` 0.173–0.234) shows the encoder does represent the realized difference *after* it happens. No change to the relational conditioner or the rollout can recover what is missing upstream of them. [30](paper/experiments/30_task_a_counterfactual_fidelity.md) |
