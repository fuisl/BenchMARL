...
| K24 | Whatever cross-agent signal the conditioned models carry survives to the rollout depth a planner uses. | **Falsified (job 1505)** | Cosine on the cross block decays monotonically with horizon and is gone by three blocks: joint +0.400 → +0.230 → **+0.015**, relational +0.408 → +0.238 → **−0.138** (anti-correlated). Both probe families agree. H1 beats H0 on 0/8 seeds at every horizon, so that falsification is horizon-robust. The relational advantage peaks at `h=2` (8/8 seeds) and **vanishes at `h=3`** (3-4/8), so K22b is a short-horizon claim. `h=3` rests on 968 survivor-selected anchors, so `E_CF` **levels** are not comparable across horizons; the cosine trend is. [31](paper/experiments/31_planning_ladder.md) |
| K22b | The relational gain tracks joint-action coverage, as M1 Row 2 predicts. | **Observed (job 1502), not a fired rule** | Relational's advantage over joint is 2.5–2.8x larger under restricted `correlated` coverage than under full `independent` coverage, in both probe families. M4 explicitly recorded this contrast as **failing to appear** and carried that as a caution against reading its rollout advantage as interaction modelling; on the repaired reference profile, measured on counterfactual response, it appears. This is a between-regime difference of paired means, which the T-A2 rule did not register — it needs its own registration before it is a claim. [30](paper/experiments/30_task_a_counterfactual_fidelity.md) |
| K29 | Observation history recovers the omitted mediating state well enough to build a world token on. | **Partial, and it saturates (job 1509)** | Recovery on cross `E_CF` rises 0.016 (single frame) → ~0.3 by `k=3` → **0.399 at `k=8`**, then flat within noise at `k=12`. No window reaches the registered 0.5 gate, against 0.871 for handing over the ball state directly. Cosine recovery is already 0.68 from one frame and rises only to 0.77, so history buys **scale**, not direction. Registered verdict: **PARTIAL — H3 is exploratory, not confirmatory.** With 16 root episodes the values across `k=3..12` are not resolvable from one another; the claim is rise-then-plateau, not an ordering. [32](paper/experiments/32_joint_representation.md) |
| K30 | Some architecture over the agents' observations can be counterfactually sufficient on Buzz Wire. | **Rejected** | The binding constraint is the observation, not the model. A single frame carries essentially no cross-agent scale information (K28, raw observation at the blind floor), history saturates near 40% (K29), and a world token pooled from same-timestep latents is a re-parameterization of an input T-A2b already showed to be insufficient. The indicated repair is to change what the agents observe, not to add machinery. [32](paper/experiments/32_joint_representation.md) |
| K28 | The counterfactual deficit is the **omitted mediating state**, not the encoder or the JEPA objective. | **Established (job 1506)** | Adding the ball/linkage state to the same latent, through the same head, recovers **86-88%** of the blind-to-ceiling gap on cross `E_CF` (0.881-0.895 -> 0.517-0.523 against a 0.4645 ceiling) and **96%** on cosine. Buzz Wire's observation is `[pos, vel, pos - goal]`, verified against the installed vmas source: no ball, no linkage, no partner, while the ball is rigidly jointed to both agents and mediates the entire cross-agent effect. This makes an agent-neutral world token the indicated repair rather than a design preference. Bounded to one task and `h=1`. [32](paper/experiments/32_joint_representation.md) |
| K25 | The T-A2 counterfactual failure is attributable to the predictor `P`. | **Rejected (job 1503)** | The effect is already lost before `P` runs. A head fitted on the current latent recovers +0.00 to +0.03 of the blind-to-ceiling gap on cross `E_CF`, while the true physical state through the identical head reaches 0.4645. Test B (`probe_floor` 0.173–0.234) shows the encoder does represent the realized difference *after* it happens. No change to the relational conditioner or the rollout can recover what is missing upstream of them. [30](paper/experiments/30_task_a_counterfactual_fidelity.md) |
